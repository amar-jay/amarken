...
def _checkpoint_valid(path: Path, config_sha256: str, tokenizer_sha256: str) -> bool:
    ...
    return metadata.get("learning_curve_config_sha256") == config_sha256 and metadata.get("tokenizer_fingerprint") == tokenizer_sha256
...
def run(config_path: Path, report_path: Path, skip_evaluation: bool = False) -> dict:
    ...
    if tokenizer.path.suffix != ".json" or tokenizer_sha256 != config["tokenizer_fingerprint"]:
        raise ValueError("learning curve is locked to the configured apostrophe-BPE artifact")
    ...
    train = PackedSequenceDataset(
        ...
        config["sequence_length"], tokenizer.eos_id(), tokenizer.pad_id(),
    )
    validation = PackedSequenceDataset(
        _tokenize(Path(config["validation_data"]), tokenizer, config["validation_token_budget"]),
        config["sequence_length"], tokenizer.eos_id(), tokenizer.pad_id(),
    )
    tokens_per_update = config["batch_size"] * config["gradient_accumulation_steps"] * config["sequence_length"]
    milestones = config["milestone_tokens"]
    if milestones != sorted(set(milestones)) or any(tokens % tokens_per_update for tokens in milestones):
        raise ValueError("milestones must be increasing exact multiples of nominal tokens/update")
    ...
    trainer = Trainer(
        ...
        TrainerConfig(
            ...
            log_every_steps=config["log_every_steps"], checkpoint_every_steps=final_updates + 1,
            output_dir=Path(config["output_dir"]), lr_schedule=config["lr_schedule"],
            warmup_steps=config["warmup_steps"], total_steps=final_updates,
            min_lr_ratio=config["min_lr_ratio"],
        ),
        OptimizerConfig(learning_rate=config["learning_rate"], weight_decay=config["weight_decay"]),
        device,
    )
    ...
    for tokens in milestones:
        ...
        validation_loss = _evaluate(trainer.model, validation, device, config["precision"])
        if not skip_evaluation and not evaluation.is_file():
            subprocess.run([
                sys.executable, "-m", "src.evaluation.meaningful_scale",
                "--checkpoint", str(checkpoint), "--tokenizer", config["tokenizer"],
                "--benchmark", config["benchmark"], "--report", str(evaluation),
        ...
        if not skip_evaluation and not legacy_evaluation.is_file():
            subprocess.run([
                ...
                "--checkpoint", str(checkpoint), "--tokenizer", config["tokenizer"],
                "--benchmark", config["legacy_benchmark"],
                "--validation", config["validation_data"],
                "--worker-output", str(legacy_evaluation),
                "--device", config["evaluation_device"],
            ], check=True)
        rows.append({
            "milestone_nominal_tokens": tokens, "optimizer_updates": target_updates,
            ...
            "checkpoint": {"path": str(checkpoint), "sha256": _sha256(checkpoint), "bytes": checkpoint.stat().st_size},
            "evaluation_v2": str(evaluation) if evaluation.is_file() else None,
            "evaluation_v1": str(legacy_evaluation) if legacy_evaluation.is_file() else None,
        })
    report = {
        "format_version": 1, "experiment_id": config["experiment_id"], "passed": len(rows) == len(milestones),
        "interpretation": "one continuous control-model learning curve; milestones are not independent restarts",
        "config_sha256": config_sha256, "tokenizer_fingerprint": tokenizer_sha256,
        ...
        "model_config": asdict(model_config), "tokens_per_update_nominal": tokens_per_update,
...
def _main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/learning_curve_dt_apostrophe_100m.json"))
    parser.add_argument("--report", type=Path, default=Path("experiments/learning_curve_dt_apostrophe_100m.json"))
    parser.add_argument("--skip-evaluation", action="store_true")
    args = parser.parse_args()
    report = run(args.config, args.report, args.skip_evaluation)
    print(json.dumps({"passed": report["passed"], "milestones": report["milestones"]}, indent=2))
