...
def _save_model_only(path: Path, trainer: Trainer, seed: int, metadata: dict) -> dict:
    ...
    payload = {
        "format_version": 1, "checkpoint_kind": "model_only_scaling",
        "model_type": trainer.model.config.model_type,
        "model_config": asdict(trainer.model.config), "model_state": trainer.model.state_dict(),
        "seed": seed, "trainer_state": asdict(trainer.state), "metadata": metadata,
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    torch.save(payload, temporary)
    os.replace(temporary, path)
    return {"path": str(path), "bytes": path.stat().st_size, "sha256": _sha256(path), "exact_resume": False}
    ...
    for scale, model_configs in config["scales"].items():
        ...
        for seed_index, seed in enumerate(config["seeds"]):
            ...
            for variant_name in ordered_names[offset:] + ordered_names[:offset]:
                ...
                initial_validation = _evaluate(model, validation_dataset, device, config["precision"], config["validation_batches"])
                if device.type == "cuda":
                ...
                final_validation = _evaluate(model, validation_dataset, device, config["precision"], config["validation_batches"])
                checkpoint = None
                ...
                results.append({
                    ...
                    "training_seconds": elapsed, "tokens_per_second": trainer.state.consumed_tokens / elapsed,
                    "optimizer_updates": trainer.state.update_step, "consumed_tokens": trainer.state.consumed_tokens,
    ...
    for scale, model_configs in config["scales"].items():
        ...
        for variant_name in variant_names:
            ...
            aggregates[f"{scale}:{variant_name}"] = {
                ...
                "validation_improvement_mean": statistics.fmean(-value["validation_loss_change"] for value in values),
                "tokens_per_second_mean": statistics.fmean(value["tokens_per_second"] for value in values),
