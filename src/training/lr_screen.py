...
def run(config_path: Path, report_path: Path) -> dict:
            ...
            results.append(
                {
                    ...
                    "final_gradient_health": {
                        k: v for k, v in records[-1].items() if k.startswith("grad_")
                    },
                    "final_ternary_statistics": {
                        k: v for k, v in records[-1].items() if k.startswith("ternary_")
                    },
                }
            )
            del trainer, model
    ...
    for variant_name in config["variants"]:
        ...
        winner = min(
            candidates,
            key=lambda row: (row["final_validation_loss"], row["learning_rate"]),
        )
        selections[variant_name] = {
            "learning_rate": winner["learning_rate"],
            "final_validation_loss": winner["final_validation_loss"],
            "median_candidate_loss": statistics.median(
                row["final_validation_loss"] for row in candidates
            ),
        }
    expected = (
    ...
    report = {
        ...
        "expected_tokens_per_candidate": expected,
