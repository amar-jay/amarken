...
def _source_paths(root: Path, source: dict) -> tuple[list[Path], Path]:
    ...
    if kind == "python_stdlib":
        ...
        paths = [path for path in base.rglob("*.py") if not excluded.intersection(path.relative_to(base).parts)]
        ...
        if kind == "lines":
            ...
            with path.open("r", encoding="utf-8", errors="strict") as handle:
                ...
                for line_number, raw in stream:
                    ...
                    yield Document(
                        id=_stable_hash(seed, f"{source['id']}:{line_number}:{content_hash}"),
                        text=text,
                        language=source["language"], domain=source["domain"],
                        source_id=source["id"], locator=locator, group_id=group,
                        content_sha256=content_hash,
                    )
        else:
            text = _normalize(path.read_text(encoding="utf-8", errors="strict"), code=True)
            if not text:
                continue
            ...
            yield Document(
                ...
                text=text, language=source["language"], domain=source["domain"],
...
def _sample(source_docs: Iterable[Document], limit: int | None, seed: str) -> list[Document]:
    if limit is None:
...
def _simhash(text: str) -> int:
    ...
    features = ("\x1f".join(tokens[index:index + 3]) for index in range(max(1, len(tokens) - 2)))
    weights = [0] * 64
...
def _deduplicate(documents: Iterable[Document], max_hamming: int) -> tuple[list[Document], Counter]:
    ...
    for document in sorted(documents, key=lambda value: value.id):
        ...
        for band in range(4):
            candidate_indexes.update(bands[(namespace, band, (fingerprint >> (band * 16)) & 0xFFFF)])
        if any((fingerprint ^ fingerprints[index]).bit_count() <= max_hamming for index in candidate_indexes):
        ...
        for band in range(4):
            bands[(namespace, band, (fingerprint >> (band * 16)) & 0xFFFF)].append(index)
    return kept, removed


def _reference_fingerprints(root: Path, patterns: list[str], ngram_size: int) -> tuple[set[str], set[int], list[dict]]:
    exact, ngrams, manifest = set(), set(), []
    paths = sorted({path for pattern in patterns for path in root.glob(pattern) if path.is_file()})
    for path in paths:
        text = _normalize(path.read_text(encoding="utf-8", errors="strict"), code=path.suffix == ".py")
        ...
        for index in range(max(0, len(tokens) - ngram_size + 1)):
            ...
            ngrams.add(int.from_bytes(hashlib.blake2b(ngram, digest_size=8).digest(), "big"))
        manifest.append({"path": str(path.relative_to(root)), "bytes": path.stat().st_size, "sha256": _sha256_file(path)})
    return exact, ngrams, manifest


def _is_contaminated(document: Document, exact: set[str], ngrams: set[int], size: int) -> bool:
    ...
    for index in range(max(0, len(tokens) - size + 1)):
        ...
        if int.from_bytes(hashlib.blake2b(raw, digest_size=8).digest(), "big") in ngrams:
            return True
    return False
...
def build(config_path: Path, output_dir: Path, root: Path = Path(".")) -> dict:
    ...
    for source in config["sources"]:
        ...
        source_manifest.append({
            **source,
            "input_files": [{
                "path": (f"cpython/Lib/{path.relative_to(source_base)}" if source["kind"] == "python_stdlib" else str(path.relative_to(root))),
                "bytes": path.stat().st_size, "sha256": _sha256_file(path),
            } for path in path_matches],
            "license_files": [{"path": path.name, "bytes": path.stat().st_size, "sha256": _sha256_file(path)} for path in license_paths if path.is_file()],
            "sampled_documents": len(sampled),
        })
    deduplicated, removed = _deduplicate(documents, config["near_duplicate_hamming_distance"])
    ...
    for document in deduplicated:
        (contaminated if _is_contaminated(document, exact_refs, ngram_refs, config["contamination_ngram_tokens"]) else clean).append(document)
    output_dir.mkdir(parents=True, exist_ok=True)
    counts: Counter = Counter()
    handles = {split: (output_dir / f"{split}.jsonl").open("w", encoding="utf-8") for split in ("train", "validation")}
    try:
        for document in sorted(clean, key=lambda value: value.id):
            split = _split(config["seed"], document.group_id, config["validation_basis_points"])
            handles[split].write(json.dumps(asdict(document), ensure_ascii=False, sort_keys=True) + "\n")
    ...
    manifest = {
        ...
        "sources": source_manifest, "contamination_references": reference_manifest,
...
def _main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/proxy_dataset.json"))
    parser.add_argument("--output-dir", type=Path, default=Path("data/processed/proxy-v1"))
    args = parser.parse_args()
    report = build(args.config, args.output_dir)
    print(json.dumps({key: report[key] for key in ("clean_documents", "contaminated_documents", "removed", "split_language_counts")}, indent=2))
    return 0
