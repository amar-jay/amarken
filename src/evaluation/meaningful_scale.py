...
def _wilson(correct: int, count: int, z: float = 1.959963984540054) -> dict:
    ...
    return {"accuracy": p, "low": center - margin, "high": center + margin, "count": count}
...
def _score_completion(model, prefix: list[int], completion: list[int]) -> tuple[float, int]:
    if not completion:
    ...
    start = len(prefix) - 1
    return sum(float(log_probs[start + offset, token]) for offset, token in enumerate(completion)), len(completion)
...
def _multiple_choice(model, tokenizer, benchmark: dict) -> tuple[list[dict], dict]:
    ...
    for task in benchmark["multiple_choice"]:
        ...
        for shift in benchmark["permutations"]:
            ...
            scores = [_score_completion(model, prompt_ids, tokenizer.encode(" " + letter))[0] for letter in LETTERS]
            ...
            details.append({
                ...
                "permutation": shift, "target_position": target_index,
                "prediction_position": prediction_index, "prediction": options[prediction_index],
                "answer": task["answer"], "correct": prediction_index == target_index,
                "target_probability": probabilities[target_index], "probabilities": probabilities,
            })
    ...
    metrics["permutation"] = {
        ...
        "all_permutations_correct": sum(all(row["correct"] for row in rows) for rows in task_groups.values()) / len(task_groups),
        "content_invariant": sum(len({row["prediction"] for row in rows}) == 1 for rows in task_groups.values()) / len(task_groups),
    }
...
def _group_accuracy(details: list[dict]) -> dict:
    ...
    return {
        f"{language}:{category}": _wilson(sum(row["correct"] for row in rows), len(rows))
...
def _generative(model, tokenizer, benchmark: dict) -> tuple[list[dict], dict]:
    ...
    for task in benchmark["generative"]:
        ...
        tensor = torch.tensor([prompt], dtype=torch.long, device=next(model.parameters()).device)
        with torch.inference_mode():
            output = model.generate(tensor, max_new_tokens=task["max_new_tokens"], temperature=0.0, eos_token_id=tokenizer.eos_id())
        ...
        details.append({
            "id": task["id"], "language": task["language"], "category": task["category"],
            "generated": generated, "normalized": normalized, "answers": task["answers"],
...
def _language_model(model, tokenizer, benchmark: dict) -> tuple[list[dict], dict]:
    ...
    for task in benchmark["language_model"]:
        score, tokens = _score_completion(model, tokenizer.encode(task["prefix"]), tokenizer.encode(task["continuation"]))
        details.append({
            "id": task["id"], "language": task["language"], "category": task["category"],
            "document_id": task["document_id"], "tokens": tokens, "nll": -score,
        })
    groups = defaultdict(list)
    for row in details:
    ...
    for language, rows in groups.items():
        ...
        metrics[language] = {"documents": len(rows), "tokens": tokens, "nll_per_token": nll / tokens, "perplexity": math.exp(min(20, nll / tokens))}
    return details, metrics


def run(checkpoint: Path, tokenizer_path: Path, benchmark_path: Path, report_path: Path, device_name: str = "cpu") -> dict:
    ...
    model = create_model(create_config(payload["model_type"], **payload["model_config"])).to(device)
    ...
    report = {
        "format_version": 2, "benchmark_id": benchmark["benchmark_id"], "passed": True,
        ...
        "tokenizer": str(tokenizer_path), "tokenizer_fingerprint": tokenizer_fingerprint(tokenizer),
...
def _main() -> int:
    ...
    parser.add_argument("--benchmark", type=Path, default=Path("benchmarks/meaningful_scale_v2.json"))
    parser.add_argument("--report", type=Path, required=True)
    parser.add_argument("--device", default="cpu")
    args = parser.parse_args()
    result = run(args.checkpoint, args.tokenizer, args.benchmark, args.report, args.device)
    print(json.dumps({"multiple_choice": result["multiple_choice"]["metrics"]["all:overall"], "generative": result["generative"]["metrics"]["all:overall"], "language_model": result["language_model"]["metrics"]}, indent=2))
    return 0
