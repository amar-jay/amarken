...
def _score_choice(
    model, prompt_ids: list[int], choice_ids: list[int], max_length: int
) -> float:
...
def _capabilities(model, processor, benchmark: dict) -> tuple[list[dict], dict]:
    ...
    for task in benchmark["tasks"]:
        ...
        prediction_index = max(
            range(len(CHOICES)), key=lambda index: (probabilities[index], -index)
        )
    ...
    metrics = {
        f"{category}:{language}": {
            ...
            "mean_target_probability": statistics.fmean(
                value["target_probability"] for value in values
            ),
        }
    ...
    for language in ("en", "tr"):
        ...
        metrics[f"overall:{language}"] = {
            "count": len(values),
            "accuracy": sum(value["correct"] for value in values) / len(values),
        }
    metrics["overall"] = {
        "count": len(details),
        "accuracy": sum(value["correct"] for value in details) / len(details),
    }
    ...
    brier = statistics.fmean(
        sum(
            (probability - (index == CHOICES.index(detail["target"]))) ** 2
            for index, probability in enumerate(detail["choice_probabilities"].values())
        )
        for detail in details
    )
    nll = statistics.fmean(
        -math.log(max(detail["target_probability"], 1e-300)) for detail in details
    )
    ece = 0.0
    for lower in (0.0, 0.2, 0.4, 0.6, 0.8):
        values = [
            detail
            for detail in details
            if lower <= detail["confidence"] < lower + 0.2
            or (lower == 0.8 and detail["confidence"] == 1.0)
        ]
        if values:
            accuracy = sum(value["correct"] for value in values) / len(values)
...
def _systems(
    model, dataset: PackedSequenceDataset, tokenizer_bytes: int, baseline_rss_bytes: int
) -> dict:
    model.eval()
    device = next(model.parameters()).device
    ...
    return {
        ...
        * 1024,
        ...
        "resident_rss_delta_from_framework_bytes": max(
            0, resident_bytes - baseline_rss_bytes
        ),
...
def _worker(
    ...
    model = create_model(create_config(model_type, **payload["model_config"])).to(
        device
    )
...
def run(
    ...
    with tempfile.TemporaryDirectory(prefix="amarken-eval-") as directory:
        for candidate in proxy["results"]:
            ...
            output = (
                Path(directory)
                / f"{variant}-seed-{candidate.get('seed', 'single')}.json"
            )
            command = [
                sys.executable,
                "-m",
    ...
    report = {
        "format_version": 1,
        "benchmark_id": json.loads(benchmark_path.read_text())["benchmark_id"],
        "passed": contamination["matching_documents"] == 0
        and all(result["capability"]["overall"]["count"] == 30 for result in results),
        ...
        "environment": {
            ...
            "tokenizer_kind": load_tokenizer(tokenizer).kind,
            "device": device,
            "torch_threads": 1,
        },
        "results": results,
...
def _main() -> int:
    ...
    for result in report["results"]:
        print(
            result["model_type"],
            result["validation_loss_full"],
            result["capability"]["overall"]["accuracy"],
        )
