...
def test_proxy_build_is_deterministic_grouped_and_contamination_clean(tmp_path):
    ...
    (tmp_path / "raw/en.txt").write_text(
        "alpha original sentence\nbeta secret benchmark phrase here\n alpha   original sentence \n",
        encoding="utf-8",
    )
    (tmp_path / "raw/tr.txt").write_text(
        "alfa özgün cümle\nbeta gizli değerlendirme cümlesi\ngama başka cümle\n",
        encoding="utf-8",
    )
    (tmp_path / "code/module.py").write_text(
        "def answer():\n    return 42\n", encoding="utf-8"
    )
    (tmp_path / "eval/heldout.txt").write_text(
        "beta secret benchmark phrase here", encoding="utf-8"
    )
    ...
    assert (
        first["outputs"]["train.jsonl"]["sha256"]
        == second["outputs"]["train.jsonl"]["sha256"]
    )
    assert (
        first["outputs"]["validation.jsonl"]["sha256"]
        == second["outputs"]["validation.jsonl"]["sha256"]
    )
    ...
    clean = _jsonl(tmp_path / "out-a/train.jsonl") + _jsonl(
        tmp_path / "out-a/validation.jsonl"
    )
    ...
    split_by_id = {
        ...
        for record in _jsonl(tmp_path / f"out-a/{split}.jsonl")
    }
    groups = {}
    for record in clean:
