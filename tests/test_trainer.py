...
def _dataset() -> PackedSequenceDataset:
    ...
    return PackedSequenceDataset(
        examples, sequence_length=8, eos_token_id=2, pad_token_id=3
    )
...
def test_packing_masks_nonassistant_padding_eos_and_record_boundaries():
    ...
    assert all(
        ...
        for segment, valid in zip(block.segment_ids, block.attention_mask)
        if not valid
    )
    assert all(
        label == -100
        for block in dataset
        for label, valid in zip(block.labels, block.attention_mask)
        if not valid
    )
    ...
    assert all(
        not (token == 11 and label == 11)
        for block in dataset
        for token, label in zip(block.input_ids, block.labels)
    )
...
def test_warmup_cosine_schedule_is_logged_at_optimizer_boundaries(tmp_path: Path):
    ...
    assert torch.allclose(
        torch.tensor(rates), torch.tensor([1.5e-4, 3e-4, 1.65e-4, 3e-5])
    )
...
def test_optimizer_groups_are_model_specific_and_complete():
    ...
    assert {group["group_name"] for group in bit_groups} == {
        "no_decay",
        "ternary_master",
    }
...
def test_exact_resume_with_accumulation_and_checkpointing(tmp_path: Path):
    ...
    torch.manual_seed(123)
    split_config = TrainerConfig(
        **{**config.__dict__, "output_dir": tmp_path / "split"}
    )
    ...
    assert _nested_equal(
        resumed.optimizer.state_dict(), continuous.optimizer.state_dict()
    )

    incompatible_config = TrainerConfig(**{**split_config.__dict__, "seed": 78})
    incompatible = Trainer(
        create_model(_glimmer_config()), _dataset(), incompatible_config
    )
    try:
