...
from src.tokenization.visualize import dataset_files, probable_mojibake, render_tokens, reservoir_samples
...
def test_reservoir_sampling_combines_synthetic_and_translation_shards(tmp_path: Path):
    ...
    shards.mkdir()
    synthetic = {
        "id": "syn-1", "language": "en", "category": "conversation",
        "messages": [{"role": "user", "content": "Hello"}, {"role": "assistant", "content": "Hi"}],
    }
    translation = {
        "id": "translation-1", "language": "tr", "category": "translation",
        "messages": [{"role": "user", "content": "Translate hello"}, {"role": "assistant", "content": "Merhaba"}],
    }
    (shards / "shard-000000.jsonl").write_text(json.dumps(synthetic) + "\n", encoding="utf-8")
    (shards / "translations-000000.jsonl").write_text(json.dumps(translation) + "\n", encoding="utf-8")
    (shards / "current.partial.json").write_text("not JSONL", encoding="utf-8")

    assert [path.name for path in dataset_files(shards)] == [
        "shard-000000.jsonl", "translations-000000.jsonl",
    ]
    ...
    assert any("User: Translate hello\nAssistant: Merhaba" in sample.text for sample in samples)
...
def test_translation_filter_and_tokenizer_round_trip_across_shards(tmp_path: Path):
    ...
    rows = [
        {
            "id": f"translation-{index}", "language": language, "category": "translation",
            "messages": [
    ...
    corpus = tmp_path / "corpus.txt"
    corpus.write_text("\n".join(message["content"] for row in rows for message in row["messages"]) * 30, encoding="utf-8")
    adapter = train_byte_bpe([corpus], 300, tmp_path / "tokenizer.json")
